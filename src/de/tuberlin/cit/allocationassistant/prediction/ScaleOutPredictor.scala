...
class ScaleOutPredictor {
  ...
  def computeScaleOut(scaleOuts: Array[Int], runtimes: Array[Double],
    ...
    println(xPredict.toArray.mkString(","))
    println(yPredict.toArray.mkString(","))
...
//    val targetMask = yPredict :< runtime
    ...
    * */
    ...
    def computeScaleOutModel(scaleOuts: Array[Int], runtimes: Array[Double],
                      scaleOutConstraint: (Int, Int), runtime: Double): Array[(Int, Double)] = {
    val x = convert(new DenseVector(scaleOuts), Double)
    val y = new DenseVector(runtimes)
    ...
    val model = xPredict.toArray.zip(yPredict.toArray)
//    println(model.map(t => "(" + t._1 + ":" + "%.2f".format(t._2) + ")").mkString(", "))
    model
